...
def generate(old_path: pathlib.Path, new_path: pathlib.Path, output_path: pathlib.Path) -> None:
    ...
    diffed_body = add_legend(diff_body(old_body, new_body))
    command = (
        f"python3 {output_path.parent / pathlib.Path(__file__).name} "
        f"--old {old_path} --new {new_path} --output {output_path}"
    )
    header = (
