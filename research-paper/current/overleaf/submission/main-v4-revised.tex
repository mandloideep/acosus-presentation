...
\begin{enumerate}
    ...
    \item We evaluated the student-facing system design in a controlled pilot study with transfer students, providing initial evidence that the interface, survey workflow and system operate as intended and are perceived as useful, easy to use, and supportive of self-advising.
\end{enumerate}
...
The \textbf{Model Layer} runs as a separate service for data normalization, prediction across all three pipeline stages, and model training. The prototype includes a transparent priority-weighted readiness calculation and supports exploratory similarity-based modeling. Asynchronous job queues keep training tasks from blocking prediction requests. Model versioning ensures that updated models are deployed only after they perform better on held-out real student data.
...
The comparison shows how different priority configurations change the calculated readiness scores. Across the ten profiles, the tested alternatives produced limited average movement from the baseline. Under equal weighting, the mean absolute difference was 1.77 points, while the largest difference was 5.24 points for one profile, whose score decreased from 78.11 to 72.87. Because the priorities are prototype configuration values and the scores are deterministic survey composites, these results describe sensitivity within this small set; they do not identify optimal priorities, validate the readiness measure as a predictor of student success, or evaluate fairness. The priorities can be re-estimated and updated as larger samples and longitudinal persistence, credit-accumulation, and completion outcomes become available.
% REV-WEIGHT-001 END
...
Behavioral intention was the weakest dimension in the evaluation (Figure~\ref{fig:behavioral}). Likelihood to Continue using the system was moderate on average ($M=3.12$, $SD=1.89$), and Likelihood to Recommend was slightly lower and similarly dispersed ($M=3.00$, $SD=1.77$). Two participants lacked usable ratings for these items, so the behavioral-intention means use $N=8$.
