%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ACOSUS Paper: Design and Feasibility Evaluation of an AI-Driven Advising Platform for Transfer Students
...
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage[hidelinks, pdftex, hypertexnames=false]{hyperref}
\hypersetup{pdftitle={ACOSUS: Design and Feasibility Evaluation of an AI-Driven Advising Platform for Transfer Students}}
\usepackage{float}
...
ACOSUS: Design and Feasibility Evaluation of an AI-Driven Advising Platform for Transfer Students
\end{center}
...
\begin{enumerate}
    ...
    \item We developed a modular progressive-learning design and implemented foundation-stage support for survey linkage, configurable model settings, readiness calculation, and exploratory modeling. Augmentation, refinement, and automatic stage transitions remain future work.
    \item We evaluated the student-facing design in a pilot study with transfer students, finding high perceived ease of use and moderate perceived usefulness for the interface and survey workflow.
\end{enumerate}
...
The \textbf{Business Logic Layer} manages application workflows and is designed to control transitions between phases of the progressive learning framework. In the intended workflow, it determines which interface the student sees at a given time: data collection only during the foundation phase, and prediction with feedback in later phases. Automatic transitions between these phases remain future work.
...
Future work & Synthetic augmentation; advanced models; automatic stage transitions; longitudinal outcome validation; fairness analysis; multi-institutional validation & Proposed; recruiting across multiple universities does not constitute institutional validation \\
\bottomrule
\end{tabular}
\end{table}
% REV-STATUS-001 END
...
\begin{itemize}
    \item \textbf{Target Survey / Readiness Measure ($Y$)}: The survey instrument that produces the current proximal, self-reported measure. In machine learning terminology, this is the \emph{label} or \emph{dependent variable}.
...
Not all factors contribute equally to student success. Without an explicit weighting mechanism, a readiness calculation would treat every survey question as equally informative, giving commute distance the same influence as academic self-efficacy. Prior research suggests that some constructs may be more strongly associated with persistence than logistical variables \parencite{factoranalysis2023}. An equal-weighting scheme would also make the calculation less interpretable for advisors, who need to understand how a particular score was generated. Prior research identifies academic, financial, institutional, and logistical factors that can shape transfer experiences \parencite{factoranalysis2023,computingtransfer2024}. ACOSUS accounts for these differences through \textbf{priority scores}. The current scores are literature-informed starting values and design choices, not coefficients estimated or validated using ACOSUS longitudinal outcomes. They can be adjusted as advisor feedback and institutional evidence clarify which factors are most relevant in a particular context (Table~\ref{tab:priority}).
...
\begin{table}[htbp]
\caption{Factor Survey priority structure used in the sensitivity analysis}
\label{tab:priority}
\centering
\begin{tabular}{@{}p{3.4cm}p{8.0cm}p{2.4cm}@{}}
\toprule
\textbf{Factor Category} & \textbf{Question Topics} & \textbf{Priority Range} \\
\midrule
Academic background & GPA, SAT score, completed credits, and course load & 7--9 \\
\addlinespace[2pt]
Financial support & Scholarship and family income support & 4 \\
\addlinespace[2pt]
Logistics and commitments & Commute distance and employment status & 6--8 \\
\addlinespace[2pt]
Interests and experience & Career aspiration, course interest, and prior experience & 4--9 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:sensitivity} reports the resulting sensitivity comparison.
...
The comparison shows how different design choices change the calculated readiness scores. Across the ten profiles, equal weighting changed scores by 1.77 points on average in absolute terms. The largest absolute within-profile difference was 5.24 points, corresponding to a decrease from 78.11 under the baseline priorities to 72.87 under equal weighting for one profile. This difference measures sensitivity to the priority assignment; it is not a change observed over time or a prediction error. These results describe sensitivity within this small set; they do not identify optimal priorities or validate the readiness measure as a predictor of student success. The priorities can be re-estimated and updated as larger samples and longitudinal persistence, credit-accumulation, and completion outcomes become available.
% REV-WEIGHT-001 END
...
Survey responses include different data types, and each type requires an appropriate normalization step before modeling, as summarized in Table~\ref{tab:normalization}. The implementation uses \textbf{cardinal} as its schema label for nominal categorical responses. The distinction between \textbf{ordinal} and \textbf{cardinal/nominal} data is especially important: ordinal responses (e.g., ``Not confident'' $<$ ``Somewhat confident'' $<$ ``Very confident'') have a meaningful order, whereas cardinal/nominal responses (e.g., career aspiration categories) do not imply ranking.
...
Cardinal/nominal & Career: ``Industry/Corporate'' & Equal weightage or domain-specific mapping \\
...
\textbf{Phase Transition Thresholds.} The configuration design allows researchers to specify how many observations would be required before the system moves from one pipeline stage to the next. Automatic transitions are not yet implemented, and any future thresholds will require empirical calibration for the selected algorithms and institutional context.
...
\textbf{Hyperparameter Tuning:} Within the implemented exploratory modeling path, configurable parameters such as distance metrics and neighbor counts support controlled comparison. Generation multipliers, network architectures, and training schedules belong to the proposed augmentation and refinement stages and remain future work.

This configuration structure is intended to allow institutions to adopt newer methods, including improved few-shot learning techniques, stronger data augmentation strategies, or newer model architectures, by updating settings rather than reengineering the full system.
...
To assess the usability and perceived value of ACOSUS, we conducted a pilot study with transfer students from public universities in the United States. This section describes the study design, evaluation instruments, quantitative results, and qualitative findings.
...
\bottomrule
\multicolumn{5}{@{}p{0.92\columnwidth}@{}}{\footnotesize\textit{Note.} Each behavioral-intention item has $N=8$. Three participants lacked at least one usable rating, including one who lacked both.} \\
\bottomrule
\end{tabular}
\end{table}
...
Behavioral intention was the weakest dimension in the evaluation (Figure~\ref{fig:behavioral}). Likelihood to Continue using the system was moderate on average ($M=3.12$, $SD=1.89$), and Likelihood to Recommend was slightly lower and similarly dispersed ($M=3.00$, $SD=1.77$). Each item had eight usable ratings. Three participants lacked at least one usable rating, including one who lacked both.

Both distributions are bimodal: several respondents scored 5/5 while others scored 1--2, with few intermediate ratings. This pattern suggests that the system appeals strongly to a subset of transfer students but not to others, rather than producing uniform mild enthusiasm. We read this cautiously: with only a small number of respondents at the foundation phase, when no predictions had yet been generated, low recommendation scores may reflect limited current utility rather than rejection of the design.
...
This paper presented ACOSUS, an AI-driven advising platform designed for transfer students in computing majors. The system addresses two problems that often limit transfer advising: fragmented information and limited institutional data for prediction.

%\textbf{Summary of Contributions.} 
Our primary contributions are: (1) a dual-survey architecture that separates readiness measurement from predictor collection; (2) structured transfer-specific data collection and organized student profiles through a unified advisor dashboard; (3) a modular progressive-learning design with implemented foundation-stage support for survey linkage, readiness calculation, configurable model settings, and exploratory modeling; and (4) a pilot evaluation with transfer students ($N=10$) showing high perceived ease of use ($M=4.13$), moderate perceived usefulness ($M=3.37$), and notable variance in behavioral intention. The dashboard is implemented, but it was not formally evaluated in the pilot. Augmentation, refinement, and automatic stage transitions remain future work.
...
The pilot's small sample size ($N=10$), small number of participating institutions, voluntary enrollment, and cross-sectional design limit generalizability, and longitudinal outcome tracking was beyond the scope of the present study. Dashboard usability, fairness, and multi-institutional validation also remain to be evaluated.
