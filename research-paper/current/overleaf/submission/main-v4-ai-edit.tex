...
\midrule
ASSIST & Official articulation and transferability repository for California public institutions and participating independent institutions & Authoritative course, general-education, and major-agreement information within its covered system & Focuses on California transfer articulation rather than collecting students' broader advising context \\
\addlinespace[3pt]
Transferology & Student-facing exploration of how courses, standardized exams, and military credits may transfer & Personalized transfer-credit matches across participating colleges and universities & Results depend on participating-institution data and center on transfer-credit decisions \\
\addlinespace[3pt]
TES & Staff-facing research, evaluation routing, equivalency management, and publication & Established workflow for institutional transfer-credit review and communication & Designed for articulation and equivalency management rather than transfer-experience data collection \\
\addlinespace[3pt]
ACOSUS & Structured transfer-student surveys, advisor-visible profiles, readiness measurement, and an AI prediction path & Combines academic and nonacademic transfer context in one advising profile & Does not provide an articulation engine; advisor usability and predictive validity have not been established \\
\bottomrule
\end{tabular}
}
\end{table}

Research prototypes complement these operational tools with algorithmic planning and advising. Examples include transfer-course planning, multiobjective degree planning, and course recommendation \parencite{nguyen2024optimal,mattei2014advising,mi2025smartcourse}. Their evaluation targets differ from ACOSUS's present focus on transfer-specific data collection, advisor-visible organization, and a progressive AI framework for small-data settings.
% REV-COMP-001 END
...
The \textbf{Business Logic Layer} manages application workflows and is designed to control transitions between phases of the progressive learning framework. The current implementation manages survey and prediction workflows; automatic transitions into the later proposed stages remain future work.
...
\begin{figure}[htbp]
\caption{Survey data processing pipeline. Raw responses undergo priority weighting and type-aware normalization to produce standardized feature vectors for readiness scoring and exploratory model evaluation.}
...
\node[plug=stage1, below=0.12cm of s1-content, text width=3cm, minimum height=0.9cm] (s1-plug) {Pluggable: Any\\lightweight similarity-based method};
