...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[CO]{\makebox[\textwidth]{Mandloi et al.\hfill ACOSUS design and feasibility evaluation}}
...
The \textbf{Model Layer} runs as a separate service for data normalization, training, and prediction. The pushed implementation contains priority-weighted readiness measurement and a post-pilot KNN path. The augmentation and refinement components described later are part of the progressive design and have not yet been implemented or evaluated.
...
Implemented after the pilot & Advisor dashboard and profiles; deterministic readiness measure; KNN training and prediction path & Verified in pushed source; not evaluated in the student pilot or in a formal advisor study \\
...
This yields a normalized score in $[0, 1]$ that reflects both the selected response and the priority of the corresponding question. The pushed Priority-Weighted Readiness Scoring (PWRS) implementation then applies a logistic calibration curve with steepness 6. PWRS aggregates questions within a multi-question Target Survey to produce the readiness measure. In the post-pilot hybrid KNN path, the same calculation is also applied to Factor Survey responses as a transparent factor-side summary supplied alongside the normalized predictor vector.

\begin{table}[htbp]
\caption{Priority scores in the pushed V2 Factor Survey.}
\label{tab:priority}
\centering
\begin{tabular}{@{}p{3.4cm}p{8.0cm}c@{}}
\toprule
\textbf{Factor Category} & \textbf{Question Topic} & \textbf{Priority} \\
\midrule
Academic background & GPA; SAT score; completed credits; courses per semester & 9; 8; 8; 7 \\
\addlinespace[2pt]
Program timeline & Program start date; expected graduation date & 8; 8 \\
\addlinespace[2pt]
Financial support & Scholarship; family income support & 4; 4 \\
\addlinespace[2pt]
Logistics and commitments & Commute distance; work status & 6; 8 \\
\addlinespace[2pt]
Interests and experience & Career aspiration; course interest; prior experience & 7; 4; 9 \\
\bottomrule
\end{tabular}
\end{table}

% REV-WEIGHT-001 BEGIN: aggregate sensitivity analysis, not weight validation.
To show how the starting priorities affect the result, we recalculated scores for the ten available deidentified profiles under three alternative scenarios. The analysis used the 11 non-date Factor Survey questions available in all ten profiles and the same normalization and logistic calibration as the pushed implementation.
...
The survey workflow, PWRS calculation, advisor-visible profiles, and a post-pilot KNN path are implemented foundation components. Augmentation, refinement, and automatic transitions between stages remain future work.

% REV-MODEL-001 BEGIN: active cautious numeric treatment.
We reproduced a leakage-safe leave-one-out diagnostic for the post-pilot KNN using the ten committed deidentified profiles, 17 encoded features, three distance-weighted neighbors, Euclidean distance, and fold-specific standardization. The KNN produced MAE 4.63, RMSE 5.91, and $R^2=-0.26$. A leave-one-out mean baseline produced MAE 4.38, RMSE 5.85, and $R^2=-0.24$. The KNN therefore did not outperform the baseline. These labels are self-reported readiness measures, not persistence, credit accumulation, degree completion, or another longitudinal outcome, so the diagnostic does not support a predictive-validity claim.
% Alternative A, methods without metrics: The pushed post-pilot implementation contains a distance-weighted KNN path with standardized inputs. Because the available labels are ten self-reported readiness measures rather than longitudinal outcomes, model performance is not treated as evidence of student-success prediction.
...
\textbf{Hyperparameter Tuning:} Within each algorithm, configurable parameters can support fine-tuning for institutional context. The current KNN path exposes settings such as distance metrics and neighbor counts. Generation multipliers, network architectures, and training schedules are proposed settings for later stages, together with logging that can support systematic model comparison.
