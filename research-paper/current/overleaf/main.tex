...
Beyond their role as predictor vectors for the ML model, Factor Surveys serve a critical function for advisors: they systematize the collection of transfer-specific information that advisors would otherwise gather through lengthy, inconsistent interviews. This design addresses the data fragmentation problem---every transfer student answers the same questions, enabling meaningful cohort-level comparisons while ensuring no critical risk factors are overlooked. Survey responses are immediately available in the advisor dashboard, eliminating the need to search emails, notes, or schedule follow-up conversations.

The system architecture permits flexible study configurations through \textbf{survey linkage}, where Target Surveys and Factor Surveys are connected through explicit associations. A single Target Survey (outcome $Y$) may be linked to multiple Factor Surveys (predictor sets $X_1, X_2, X_3$), enabling researchers to investigate which predictor combinations best predict success. This linkage architecture provides several research advantages:
...
\begin{tikzpicture}[
    ...
    \node[font=\small\bfseries, text=red!70] at (0, 3.5) {TARGET SURVEYS (Outcome $Y$)};
    ...
    \node[font=\small\bfseries, text=blue!70] at (8, 3.5) {FACTOR SURVEYS (Predictors $X$)};
    ...
    \node[innerbox=teal!60, fill=teal!20] (normalize) at (1.6, -4) {\textbf{Predictor}\\Normalization};
...
\caption{Dual-Survey Architecture with survey linkage. Target Surveys collect the outcome ($Y$), Factor Surveys collect predictors ($X$). Dashed lines show linkage relationships enabling flexible study configurations.}
