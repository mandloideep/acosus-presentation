...
\begin{abstract}
\normalsize
[ABSTRACT PLACEHOLDER: 100-200 words summarizing the ACOSUS system, its dual-survey architecture, progressive learning framework, and pilot evaluation results.]\vskip 2mm
...
To address this gap, we designed and evaluated an intelligent advising system that merges academic records with social-cognitive, financial, and experiential data collected both before and after transfer. Our contributions are three-fold:

\begin{enumerate}
    \item \textbf{First}, we introduce the first intelligent advising platform designed specifically for transfer students, integrating standard academic records (e.g., transfer-credit counts, GPA history, enrollment status) with student-provided background and goal information to build a unified longitudinal profile.
    \item \textbf{Second}, we embed a transparent, off-the-shelf classification module that converts these profiles into clear, interpretable predictions of each student's academic persistence and career-readiness prospects.
    \item \textbf{Third}, we validate the platform through a pilot deployment with transfer students, who provide feedback on their experience using it, showing that it streamlines self-advising workflows and is perceived as trustworthy, actionable, and equity-enhancing.
\end{enumerate}
...
Technology-mediated solutions have begun to emerge, but most commercial platforms were built for native students. Predictive-analytics tools typically ingest registrar feeds and LMS clickstreams, rarely accounting for variables such as credit-completion velocity or social-belonging indicators that matter disproportionately for transfer cohorts \cite{acosus2024}. Reviews warn that models calibrated on homogeneous populations risk biased predictions and may inadvertently disadvantage students from minoritized backgrounds \cite{denley2016}. Only a handful of prototype systems explicitly target transfer populations, and these remain at pilot scale \cite{bosse2019}.
...
To address the data constraints and advising challenges outlined in Section~\ref{sec:introduction}, we designed ACOSUS (AI-driven Counseling System for Underrepresented Students). This section presents the system's core architectural innovations: a dual-survey architecture that captures transfer-specific variables while supporting advisor workflows (\S\ref{subsec:dualsurvey}), and a progressive learning framework designed for the small-data regime inherent in departmental-level transfer advising (\S\ref{subsec:progressive}).

\subsection{The Dual-Survey Architecture}\label{subsec:dualsurvey}

The variables most predictive of transfer success---credit articulation outcomes, ``transfer shock'' severity, belonging uncertainty, and financial precarity---are seldom captured in standard institutional systems \cite{topicmodeling2024,computingtransfer2024,socialcognitive2022}. Furthermore, academic advisors lack unified access to the information they need; prior research documents that advisors spend significant time gathering student data from disparate sources before providing meaningful guidance \cite{preposttransfer2014,harper2019}. ACOSUS addresses both challenges through a Dual-Survey Architecture that simultaneously feeds machine learning models and consolidates advisor-facing information.

Prior research on transfer student success has identified multiple dimensions that contribute to successful degree completion, including academic self-efficacy, institutional commitment, social integration, and career goal clarity \cite{factoranalysis2023,topicmodeling2024,computingtransfer2024}. Our earlier factor analysis work identified clusters of social-cognitive variables---academic confidence, time management, financial stability, and support systems---that distinguish successful transfer students from those who struggle [CITE-PLACEHOLDER]. Building on these findings, ACOSUS operationalizes ``success'' as representing the student's likelihood of academic success across these dimensions and ``factors'' as determinants that affect the likelihood.

ACOSUS implements this predictive framework through a Dual-Survey Architecture that cleanly separates label collection from feature collection. \textbf{Target Surveys} capture success through two supported modes: (1) a single direct self-assessment question where students rate their success on a 0--100 scale, or (2) a multi-question instrument measuring constructs such as academic confidence, commitment, time management self-efficacy, and career motivation, which are aggregated into a single score via Weighted Calculation. \textbf{Factor Surveys} collect the independent variables used for prediction, organized into categories: academic background (pre-transfer GPA, credits transferred, standardized test scores), financial circumstances (scholarship status, family support, employment intensity), logistical factors (commute distance, work hours), and interest/experience indicators (career aspiration, prior subject experience).

Beyond their role as feature vectors, Factor Surveys serve a critical function for advisors: they systematize the collection of transfer-specific information that advisors would otherwise gather through lengthy, inconsistent interviews. This dual-purpose design addresses the data fragmentation problem---every transfer student answers the same questions, enabling meaningful cohort-level comparisons while ensuring no critical risk factors are overlooked. Survey responses are immediately available in the advisor dashboard, eliminating the need to search emails, notes, or schedule follow-up conversations.

The system architecture permits flexible study configurations through \textbf{survey linkage}, where Target Surveys and Factor Surveys are connected through explicit associations. A single Target Survey (measuring $Y$) may be linked to multiple Factor Surveys (measuring different feature sets $X_1, X_2, X_3$), enabling researchers to investigate which feature combinations best predict success. This linkage architecture provides several research advantages:

\begin{enumerate}
    \item \textbf{Comparative feature analysis}---researchers can deploy alternative Factor Surveys to the same cohort and compare predictive validity across different feature sets;
...
\begin{tikzpicture}[node distance=0.4cm and 0.6cm]
    % Target Surveys Group
    \node[surveybox, fill=targetsurvey] (ts1) {Single-Question\\Direct 0--100 Scale};
    \node[surveybox, fill=targetsurvey, below=of ts1] (ts2) {Multi-Question\\Weighted Aggregated};

    \begin{scope}[on background layer]
        \node[groupbox, fill=targetsurvey!30, fit=(ts1)(ts2), inner sep=8pt, label={[font=\small\bfseries]above:TARGET SURVEYS (Label: $Y$)}] (targetgroup) {};
    \end{scope}

    % Factor Surveys Group
    \node[surveybox, fill=factorsurvey, right=2.5cm of ts1] (fs1) {Academic Background\\GPA, SAT, Credits};
    \node[surveybox, fill=factorsurvey, below=of fs1] (fs2) {Financial Support\\Scholarship, Income};
    \node[surveybox, fill=factorsurvey, below=of fs2] (fs3) {Logistics\\Distance, Work Hours};
    \node[surveybox, fill=factorsurvey, below=of fs3] (fs4) {Interest \& Experience\\Career, Prior Exp.};
    \node[surveybox, fill=factorsurvey!50, draw=black!40, dashed, below=of fs4] (fs5) {[Future Surveys]\\Adaptable};

    \begin{scope}[on background layer]
        \node[groupbox, fill=factorsurvey!30, fit=(fs1)(fs2)(fs3)(fs4)(fs5), inner sep=8pt, label={[font=\small\bfseries]above:FACTOR SURVEYS (Features: $X$)}] (factorgroup) {};
    \end{scope}

    % Processing Group
    \node[surveybox, fill=processing, right=2.5cm of fs2] (norm) {Feature\\Normalization\\(0--10 scale)};
    \node[surveybox, fill=processing, below=of norm] (pwrs) {Weighted\\Calculation\\(if multi-Q)};

    \begin{scope}[on background layer]
        \node[groupbox, fill=processing!30, fit=(norm)(pwrs), inner sep=8pt, label={[font=\small\bfseries]above:DATA PROCESSING}] (procgroup) {};
    \end{scope}

    % Output Group
    \node[surveybox, fill=outputgreen, right=2.5cm of norm] (model) {$Y = f(X)$\\Predict Readiness};
    \node[surveybox, fill=outputgreen, below=of model] (score) {Readiness Score\\(0--1 continuous)};

    \begin{scope}[on background layer]
        \node[groupbox, fill=outputgreen!30, fit=(model)(score), inner sep=8pt, label={[font=\small\bfseries]above:ML MODEL}] (outgroup) {};
    \end{scope}

    % Linkage arrows (dashed)
    \draw[dashedarrow] (ts1.east) -- ++(0.3,0) |- (fs1.west);
    \draw[dashedarrow] (ts1.east) -- ++(0.3,0) |- (fs2.west);
    \draw[dashedarrow] (ts1.east) -- ++(0.3,0) |- (fs3.west);
    \draw[dashedarrow] (ts2.east) -- ++(0.5,0) |- (fs1.west);
    \draw[dashedarrow] (ts2.east) -- ++(0.5,0) |- (fs3.west);
    \draw[dashedarrow] (ts2.east) -- ++(0.5,0) |- (fs4.west);

    % Data flow arrows (solid)
    \draw[arrow] (fs1.east) -- ++(0.4,0) |- (norm.west);
    \draw[arrow] (fs2.east) -- ++(0.4,0) |- (norm.west);
    \draw[arrow] (fs3.east) -- ++(0.4,0) |- (norm.west);
    \draw[arrow] (fs4.east) -- ++(0.4,0) |- (norm.west);
    \draw[arrow] (ts2.east) -- ++(1.8,0) |- (pwrs.west);

    \draw[arrow] (norm.east) -- (model.west);
    \draw[arrow] (pwrs.east) -- ++(0.4,0) |- (model.west);
    \draw[arrow] (ts1.east) -- ++(2.0,0) |- (model.west);
    \draw[arrow] (model) -- (score);

    % Legend
    \node[below=1.2cm of fs5, font=\tiny, text width=6cm, align=left] {
        \tikz{\draw[dashedarrow] (0,0) -- (0.5,0);} Survey linkage \quad
...
\begin{tikzpicture}[node distance=0.5cm and 1cm]
    % Stage 1
    \node[stagebox, fill=stage1green, draw=stage1border] (stage1) {
        \textbf{STAGE 1: FOUNDATION}\\[2pt]
        Data Collection \&\\Initial Prediction\\[4pt]
        {\tiny\textbullet\ Collect labeled observations}\\
        {\tiny\textbullet\ Establish ground truth}\\
        {\tiny\textbullet\ Similarity-based predictions}\\[4pt]
        {\tiny\textit{Pluggable: Any similarity method}}
    };

    % Stage 2
    \node[stagebox, fill=stage2yellow, draw=stage2border, right=1.2cm of stage1] (stage2) {
        \textbf{STAGE 2: AUGMENTATION}\\[2pt]
        Synthetic Data\\Generation\\[4pt]
        {\tiny\textbullet\ Learn distributions}\\
        {\tiny\textbullet\ Synthesize observations}\\
        {\tiny\textbullet\ Preserve statistics}\\[4pt]
        {\tiny\textit{Pluggable: Any generative technique}}
    };

    % Stage 3
    \node[stagebox, fill=stage3blue, draw=stage3border, right=1.2cm of stage2] (stage3) {
        \textbf{STAGE 3: REFINEMENT}\\[2pt]
        Advanced Non-Linear\\Modeling\\[4pt]
        {\tiny\textbullet\ Train on augmented corpus}\\
        {\tiny\textbullet\ Capture complex relationships}\\
        {\tiny\textbullet\ Continuous improvement}\\[4pt]
        {\tiny\textit{Pluggable: Any DL architecture}}
    };

    % Input
    \node[surveybox, fill=factorsurvey, left=1.2cm of stage1] (input) {Survey Responses\\(Features $X$,\\Labels $Y$)};

    % Output
    \node[surveybox, fill=outputgreen, right=1.2cm of stage3] (output) {Readiness Score\\(0--1 continuous)};

    % Feedback
    \node[surveybox, fill=processing, below=1.5cm of stage2] (feedback) {Student Feedback\\$\rightarrow$ Model Improvement};

    % Framework box
    \begin{scope}[on background layer]
        \node[groupbox, fill=framegray, draw=frameborder, fit=(stage1)(stage2)(stage3), inner sep=12pt, label={[font=\small\bfseries]above:ACOSUS THREE-STAGE PROGRESSIVE PIPELINE}] (framework) {};
    \end{scope}

    % Arrows between stages
    \draw[arrow] (input) -- (stage1);
    \draw[arrow] (stage1) -- node[above, font=\tiny] {Corpus established} (stage2);
    \draw[arrow] (stage2) -- node[above, font=\tiny] {Volume achieved} (stage3);
    \draw[arrow] (stage3) -- (output);

    % Feedback arrows
    \draw[arrow] (output) -- ++(0,-0.8) -| (feedback);
    \draw[dashedarrow] (feedback) -- ++(-3.5,0) |- node[pos=0.25, right, font=\tiny] {New observations} (stage1.south);
    \draw[dashedarrow] (feedback) -- ++(3.5,0) |- node[pos=0.25, left, font=\tiny] {Validation} (stage3.south);
...
\begin{tikzpicture}[node distance=0.4cm and 1.5cm]
    % Survey Collection
    \node[surveybox, fill=factorsurvey] (q1) {Question 1\\(Ordinal)};
    \node[surveybox, fill=factorsurvey, below=of q1] (q2) {Question 2\\(Cardinal)};
    \node[surveybox, fill=factorsurvey, below=of q2] (q3) {Question 3\\(Continuous)};

    \begin{scope}[on background layer]
        \node[groupbox, fill=factorsurvey!30, fit=(q1)(q2)(q3), inner sep=10pt, label={[font=\small\bfseries]above:SURVEY COLLECTION}] (collgroup) {};
    \end{scope}

    % Processing
    \node[surveybox, fill=targetsurvey, right=2cm of q1] (pw) {Priority\\Weighting};
    \node[surveybox, fill=targetsurvey, below=0.6cm of pw] (tn) {Type-Aware\\Normalization};

    \begin{scope}[on background layer]
        \node[groupbox, fill=targetsurvey!30, fit=(pw)(tn), inner sep=10pt, label={[font=\small\bfseries]above:DATA PROCESSING}] (procgroup) {};
    \end{scope}

    % Output
    \node[surveybox, fill=outputgreen, right=2cm of pw, yshift=-0.5cm] (fv) {Normalized\\Feature Vector\\(0--10 scale)};

    \begin{scope}[on background layer]
        \node[groupbox, fill=outputgreen!30, fit=(fv), inner sep=10pt, label={[font=\small\bfseries]above:MODEL INPUT}] (outgroup) {};
    \end{scope}

    % Arrows
    \draw[arrow] (q1.east) -- ++(0.5,0) |- (pw.west);
    \draw[arrow] (q2.east) -- ++(0.5,0) |- (pw.west);
    \draw[arrow] (q3.east) -- ++(0.5,0) |- (pw.west);
    \draw[arrow] (pw) -- (tn);
    \draw[arrow] (tn.east) -- ++(0.5,0) |- (fv.west);
...
The ACOSUS platform implements a four-layer architecture that separates concerns across presentation, business logic, data persistence, and machine learning (Figure~\ref{fig:architecture}). This separation enables independent scaling of each layer and supports the system's dual purpose: serving student-facing predictions while centralizing advisor-facing information.
...
\begin{tikzpicture}[node distance=0.3cm and 0.4cm]
    % Presentation Layer
    \node[layerbox, fill=factorsurvey, minimum width=2.5cm] (student) {Student Portal\\Survey | Prediction};
    \node[layerbox, fill=factorsurvey, right=of student, minimum width=2.5cm] (advisor) {Advisor Portal\\Search | Dashboard};
    \node[layerbox, fill=factorsurvey, right=of advisor, minimum width=2.5cm] (admin) {Admin Portal\\Configure | Train};

    \begin{scope}[on background layer]
        \node[groupbox, fill=factorsurvey!30, fit=(student)(advisor)(admin), inner sep=8pt, label={[font=\small\bfseries]above:PRESENTATION LAYER}] (presgroup) {};
    \end{scope}

    % Business Logic Layer
    \node[layerbox, fill=targetsurvey, below=1.2cm of student] (auth) {Authentication};
    \node[layerbox, fill=targetsurvey, right=of auth] (survey) {Survey\\Management};
    \node[layerbox, fill=targetsurvey, right=of survey] (workflow) {Phase\\Orchestration};
    \node[layerbox, fill=targetsurvey, right=of workflow] (trigger) {Training\\Triggers};

    \begin{scope}[on background layer]
        \node[groupbox, fill=targetsurvey!30, fit=(auth)(survey)(workflow)(trigger), inner sep=8pt, label={[font=\small\bfseries]above:BUSINESS LOGIC LAYER}] (bizgroup) {};
    \end{scope}

    % Data Layer
    \node[layerbox, fill=outputgreen, below=1.2cm of auth, minimum width=3cm] (db) {Document Storage};
    \node[layerbox, fill=outputgreen, right=0.8cm of db, minimum width=5cm] (coll) {Users | Surveys | Responses | Models};

    \begin{scope}[on background layer]
        \node[groupbox, fill=outputgreen!30, fit=(db)(coll), inner sep=8pt, label={[font=\small\bfseries]above:DATA LAYER}] (datagroup) {};
    \end{scope}

    % Model Layer
    \node[layerbox, fill=modelpink, below=1.2cm of db] (norm) {Data\\Normalizer};
    \node[layerbox, fill=modelpink, right=of norm] (predict) {Prediction\\Service};
    \node[layerbox, fill=modelpink, right=of predict] (train) {Training\\Pipeline};
    \node[layerbox, fill=modelpink, right=of train] (version) {Model\\Versioning};

    \begin{scope}[on background layer]
        \node[groupbox, fill=modelpink!30, fit=(norm)(predict)(train)(version), inner sep=8pt, label={[font=\small\bfseries]above:MODEL LAYER}] (mlgroup) {};
    \end{scope}

    % Arrows between layers
    \draw[arrow] (presgroup.south) -- node[right, font=\tiny] {REST API} (bizgroup.north);
    \draw[arrow] (bizgroup.south) -- (datagroup.north);
    \draw[arrow] ($(bizgroup.south)+(1.5,0)$) -- node[right, font=\tiny] {Async} ($(mlgroup.north)+(1.5,0)$);
...
% TODO: Add Figure 7 - TAM construct scores
\begin{figure}[htbp]
\centering
% \includegraphics[width=0.8\textwidth]{fig7_tam_metrics.png}
\fbox{\parbox{0.8\textwidth}{\centering [FIGURE PLACEHOLDER: TAM construct scores showing Perceived Usefulness and Perceived Ease of Use.]}}
...
% TODO: Add Figure 10 - Behavioral intention metrics
\begin{figure}[htbp]
\centering
% \includegraphics[width=0.8\textwidth]{fig10_behavioral_intention.png}
\fbox{\parbox{0.8\textwidth}{\centering [FIGURE PLACEHOLDER: Behavioral intention metrics showing likelihood to continue and recommend.]}}
...
% TODO: Add Figure 16 - Qualitative theme distribution
\begin{figure}[htbp]
\centering
% \includegraphics[width=0.8\textwidth]{fig16_qualitative_themes.png}
\fbox{\parbox{0.8\textwidth}{\centering [FIGURE PLACEHOLDER: Qualitative theme distribution from open-ended responses.]}}
\caption{Qualitative theme distribution. Usability and Satisfaction were the most frequently occurring themes (3 mentions each), followed by Efficiency and Personalization.}
\label{fig:themes}
\end{figure}
...
\textbf{Third}, we validated the platform through a pilot deployment during the foundation phase (Section~\ref{sec:evaluation}). With $N=4$ transfer students from a public urban university, we assessed usability and data collection quality prior to model training. Perceived Ease of Use achieved a perfect score ($M=5.00$), indicating that the survey workflow successfully balanced comprehensiveness with accessibility---a critical requirement for time-constrained transfer students. The absence of reported technical or accessibility issues indicates deployment readiness for broader recruitment.

\subsection{Current Status and Limitations}

The system is currently operational and collecting labeled observations during the foundation phase. The pilot evaluation validated the data collection workflow and established that the priority-weighted formula produces labels that resonate with student self-perceptions. However, the predictive components of the framework---similarity-based predictions in the foundation stage, synthetic data generation in the augmentation stage, and deep learning in the refinement stage---remain to be evaluated as the training corpus grows.

Several limitations constrain interpretation of these findings:

\begin{itemize}
    \item \textbf{Sample Size:} With $N=4$, statistical inference is limited. The results represent formative usability feedback rather than generalizable effectiveness claims. Effect sizes and variance estimates are unstable at this sample size.
    \item \textbf{Self-Selection Bias:} Participants voluntarily enrolled after email invitation, potentially over-representing students with positive attitudes toward technology-assisted advising.
    \item \textbf{Single Institution:} All participants were from one urban public university. Generalizability to other institutional contexts (community colleges, rural institutions, private universities) remains untested.
    \item \textbf{Short-Term Assessment:} The study captured immediate post-use perceptions. Longitudinal assessment of actual academic outcomes (GPA changes, persistence, graduation) was beyond the pilot scope.
    \item \textbf{No Control Condition:} Without a comparison group using traditional advising, we cannot attribute satisfaction to ACOSUS-specific features versus general positive response to any structured advising interaction.
\end{itemize}
...
\textbf{Continued Recruitment and Multi-Institutional Deployment.} The immediate priority is recruiting additional participants to reach the augmentation stage threshold, enabling evaluation of the full progressive pipeline. Concurrent deployment across partner institutions---including community colleges and regional universities---will assess generalizability across diverse transfer pathways and institutional contexts.

\textbf{Longitudinal Outcome Tracking.} The pilot captured perceptions at a single time point. Future work will track participants' academic trajectories (GPA changes, persistence, time-to-degree) to correlate predicted readiness scores with actual outcomes once sufficient training data enables predictions.

\textbf{Advisor Perspective Evaluation.} The current evaluation focused on student-facing workflows. A complementary study will assess whether the unified advisor dashboard reduces information-gathering burden and improves advising efficiency, addressing the data fragmentation problem identified in prior research \cite{preposttransfer2014,harper2019}.

\textbf{Comparative Effectiveness Study.} With sufficient sample size, a controlled study comparing ACOSUS-assisted advising against traditional advising will establish whether the system improves outcomes beyond what structured advising interactions alone provide.

\textbf{Algorithm Comparison Studies.} The pluggable architecture enables systematic comparison of alternative algorithms within each pipeline stage. Future work will evaluate which specific similarity metrics, generative techniques, and neural architectures perform best for transfer student prediction, contributing empirical guidance to the broader educational data mining community.
