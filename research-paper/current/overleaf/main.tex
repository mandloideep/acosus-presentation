...
% Figure 3 - Dual-Survey Architecture diagram (Whimsical-style)
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    % Styles for inner boxes (dashed borders)
    innerbox/.style={rectangle, rounded corners=8pt, minimum width=2.4cm, minimum height=1.2cm, text centered, draw=#1, dashed, line width=1pt, font=\small, text width=2.2cm, align=center},
    % Styles for outer group containers (solid borders)
    outerbox/.style={rectangle, rounded corners=10pt, draw=#1, line width=1.5pt},
    % Arrow styles
    linearrow/.style={-{Stealth[length=3mm, width=2mm]}, line width=1.5pt, color=gray!70},
    dashedlink/.style={-{Stealth[length=2.5mm, width=2mm]}, line width=1.2pt, color=gray!60, dashed},
]

    % ===== TARGET SURVEYS (Top Left) =====
    \node[font=\small\bfseries, text=red!70] at (0, 3.5) {TARGET SURVEYS (Label)};

    \node[innerbox=orange!70, fill=orange!15] (ts1) at (0, 2) {\textbf{Single-Question}};
    \node[innerbox=orange!70, fill=orange!15] (ts2) at (0, 0.3) {\textbf{Multi-Question}};

    \begin{scope}[on background layer]
        \node[outerbox=red!40, fill=white, fit=(ts1)(ts2), inner sep=12pt] (targetgroup) {};
    \end{scope}

    % ===== FACTOR SURVEYS (Top Right - 2x2 Grid) =====
    \node[font=\small\bfseries, text=blue!70] at (8, 3.5) {FACTOR SURVEYS (Features)};

    \node[innerbox=blue!50, fill=blue!10] (fs1) at (6.2, 2) {\textbf{Logistics \&}\\Commitment\\{\scriptsize Distance, Work Hours}};
    \node[innerbox=blue!50, fill=blue!10] (fs2) at (9.8, 2) {\textbf{[Future}\\Surveys]\\{\scriptsize Adaptable to New Predictors}};
    \node[innerbox=blue!50, fill=blue!10] (fs3) at (6.2, 0.3) {\textbf{Academic}\\Background\\{\scriptsize GPA, SAT, Credits}};
    \node[innerbox=blue!50, fill=blue!10] (fs4) at (9.8, 0.3) {\textbf{Financial}\\Support\\{\scriptsize Scholarship, Family Income}};

    \begin{scope}[on background layer]
        \node[outerbox=blue!60, fill=white, fit=(fs1)(fs2)(fs3)(fs4), inner sep=12pt] (factorgroup) {};
    \end{scope}

    % ===== DATA PROCESSING (Bottom Left - aligned with Target) =====
    \node[font=\small\bfseries, text=purple!70] at (0, -2.5) {DATA PROCESSING};

    \node[innerbox=teal!60, fill=teal!20] (weighted) at (-1.2, -4) {\textbf{Weighted}\\Calculation\\{\scriptsize (if multi-question)}};
    \node[innerbox=teal!60, fill=teal!20] (normalize) at (1.6, -4) {\textbf{Feature}\\Normalization};

    \begin{scope}[on background layer]
        \node[outerbox=purple!60, fill=white, fit=(weighted)(normalize), inner sep=12pt] (procgroup) {};
    \end{scope}

    % ===== ML MODEL (Bottom Right - aligned with Factor) =====
    \node[font=\small\bfseries, text=purple!70] at (8, -2.5) {ML MODEL};

    \node[innerbox=pink!70, fill=pink!25] (predict) at (6.5, -4) {\textbf{Predict}\\Readiness};
    \node[innerbox=red!40, fill=red!15] (score) at (9.5, -4) {\textbf{Readiness}\\Score};

    \begin{scope}[on background layer]
        \node[outerbox=pink!60, fill=white, fit=(predict)(score), inner sep=12pt] (mlgroup) {};
    \end{scope}

    % ===== CONNECTIONS =====

    % Dashed link from Target to Factor with "Link" label (horizontal connection)
    \coordinate (linkpoint) at (3, 1.15);
    \draw[dashedlink] (ts1.east) -- ++(0.8,0) |- (fs1.west);
    \draw[dashedlink] (ts1.east) -- ++(0.6,0) |- (fs3.west);
    \draw[dashedlink] (ts2.east) -- ++(1.0,0) |- (fs1.west);
    \draw[dashedlink] (ts2.east) -- ++(0.8,0) |- (fs3.west);
    % Link label
    \node[font=\small, text=gray!70] at (3, 2.3) {Link};

    % Flow from Target to Weighted Calculation
    \draw[linearrow, rounded corners=8pt] (targetgroup.south) -- ++(0,-0.5) -- (weighted.north);

    % Flow from Factor to Feature Normalization
    \draw[linearrow, rounded corners=8pt] (factorgroup.south) -- ++(0,-0.3) -| (normalize.north);

    % Flow within Data Processing
    \draw[linearrow] (weighted.east) -- (normalize.west);

    % Flow from Data Processing to ML Model
    \draw[linearrow] (procgroup.east) -- (mlgroup.west);

    % Flow within ML Model
    \draw[linearrow] (predict.east) -- (score.west);
