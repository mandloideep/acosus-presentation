...
\node[trainbox,minimum width=32mm,minimum height=18mm,right=18mm of target.south east,anchor=north west] (weighted) {Weighted Calculation\\(if multi-question)};
\node[trainbox,minimum width=32mm,minimum height=18mm,below=6mm of weighted] (normtrain) {Feature Normalization\\(Training)};
\node[predbox,minimum width=32mm,minimum height=18mm,below=6mm of normtrain] (normpred) {Feature Normalization\\(Prediction)};
...
\node[trainbox,minimum width=32mm,minimum height=16mm,right=18mm of weighted.north east,anchor=north west] (train) {Train Model};
\node[trainbox,minimum width=32mm,minimum height=16mm,below=6mm of train] (save) {Save Model};
\node[predbox,minimum width=32mm,minimum height=16mm,below=12mm of save] (load) {Load Trained Model};
\node[predbox,minimum width=32mm,minimum height=16mm,below=6mm of load] (predict) {Predict Readiness};
...
% Legend
\node[below=12mm of fbox.south west,anchor=north west] (legendtitle) {};
\draw[trainarr] (legendtitle.east) -- ++(12mm,0) node[right,font=\small,black] {Training Flow};
\draw[predarr] ([yshift=-8mm]legendtitle.east) -- ++(12mm,0) node[right,font=\small,black] {Prediction Flow};
\end{tikzpicture}
}%
...
This paper presented ACOSUS, an AI-driven counseling system designed for transfer students in computing majors, addressing fragmented information landscapes and data scarcity in departmental-level advising.

\textbf{Summary of Contributions.} Our work makes three contributions: (1) a Dual-Survey Architecture that separates outcome measurement from feature collection while providing advisors with structured, standardized student profiles through a unified dashboard; (2) a modular prediction architecture with an algorithm-agnostic design that enables researchers to empirically evaluate alternative approaches on their specific transfer student populations, with configuration options for survey linkage, phase transitions, and hyperparameters supporting adaptation to diverse institutional contexts; and (3) pilot validation with transfer students ($N=4$) demonstrating strong usability---Perceived Ease of Use achieved a perfect score, indicating the survey workflow balances comprehensiveness with accessibility.

\textbf{Limitations and Current Status.} The system is operational and collecting observations during the foundation phase, but predictive components remain to be evaluated as the corpus grows. The pilot's small sample size ($N=4$), single-institution setting, voluntary enrollment, and cross-sectional design limit generalizability; longitudinal outcome tracking was beyond scope.

\textbf{Future Work.} Priorities include: (1) evaluating the full progressive pipeline once sufficient observations accumulate, including accuracy, calibration, and fairness assessment; (2) multi-institutional deployment across community colleges and regional universities to test generalizability; (3) integrating advisor feedback mechanisms to refine predictions and assessing dashboard utility; and (4) exploring LLM integration for personalized narrative explanations and actionable intervention recommendations.

The modular architecture, with its emphasis on immediate advisor utility during data collection and configuration-based adaptation, offers a generalizable template for deploying intelligent systems in small-data educational contexts.
