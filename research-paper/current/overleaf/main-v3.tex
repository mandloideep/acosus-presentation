...
% DSI compliance: the current abstract below is ~180 words; DSI requires <=100 words.
% Proposed trimmed rewrite (97 words) for discussion -- review and replace the
% paragraph below with this version if you want to adopt it:
%
% Transfer students face advising challenges that conventional systems do not
% address well: records are split across institutions, and predictive models are
% typically trained on native-cohort students. We present ACOSUS, an advising
% platform that combines a dual-survey design separating outcome measurement from
% predictor collection with a progressive learning framework for low-data
% settings. A pilot with ten transfer students at six public U.S. universities
% provides initial evidence that the platform operates as intended; participants
% endorsed the system positively on ease of use and usefulness. The evaluation
% demonstrates design feasibility; the system is now collecting data for later
% predictive evaluation.
%
% __ALT_PHRASING__: Abstract evaluation sentence (alternative form that adds the
% foundation-phase caveat; longer but more cautious). To use, replace the
% sentence "participants endorsed the system positively on perceived ease of use
% and perceived usefulness, and the survey workflow operated correctly" below
% with:
%   participants endorsed the system positively on perceived ease of use and
%   perceived usefulness, with the lower usefulness rating consistent with the
%   foundation-phase context in which no predictions had yet been generated,
%   and the survey workflow operated correctly.
%
% __ALT_PHRASING__: Abstract participating-institutions framing (active version
% below is recommended Option A: "six public universities in the United
% States"). Alternative phrasing for reviewer consideration:
%
% Option B (leverages the MSI/HSI alignment with the paper's underrepresented-
% students framing; replace "six public universities in the United States" with):
%   six public U.S. universities, several of them minority-serving institutions
%
% Option C would name each institution explicitly in the Study Design section
% rather than in the abstract; the abstract phrasing would remain as Option A.
\begin{center}
\textbf{ABSTRACT}
\end{center}

Transfer students, particularly those from underrepresented backgrounds in computing, face advising challenges that conventional systems do not address well. Their records are split across institutions, receiving universities often have limited visibility into pre-transfer coursework, and many predictive models are trained on students who entered as freshmen. We present ACOSUS (AI-driven counseling system for transfer students), an advising platform built for transfer student populations. This paper describes the design rationale, architecture, and implementation of the system. The system combines a dual-survey design that separates outcome measurement from predictor collection with a progressive learning framework for settings where little institutional data are available at the start. A controlled pilot study with transfer students at six public universities in the United States provides initial evidence that the design works as intended: participants endorsed the system positively on perceived ease of use and perceived usefulness, and the survey workflow operated correctly. The evaluation demonstrates design feasibility rather than validated predictive performance; the system is now deployed and collecting data for later evaluation at scale.
...
\begin{enumerate}
    ...
    \item We evaluated the system design in a controlled pilot study with transfer students, providing initial evidence that the platform operates as intended and is perceived as useful, easy to use, and supportive of self-advising.
\end{enumerate}
...
This section introduces the terms used throughout the paper. ACOSUS treats transfer student success prediction as a supervised learning problem in which an outcome is predicted from a set of input variables. Because the platform separates what is being predicted from the information used to make that prediction, three terms are central to the design:

\begin{itemize}
    \item \textbf{Target Survey / Outcome ($Y$)}: The survey instrument that measures what we want to predict, namely the student's likelihood of academic success. In machine learning terminology, this is the \emph{label} or \emph{dependent variable}.
...
These definitions matter because many of the factors that shape transfer success do not map cleanly onto the information universities already store. Variables such as credit articulation outcomes, transfer shock, belonging, and financial precarity are rarely captured in standard institutional systems \cite{topicmodeling2024,computingtransfer2024,socialcognitive2022}. Advisors also often lack one place to access the information they need; prior work shows that they spend substantial time gathering student data from multiple sources before they can offer useful guidance \cite{preposttransfer2014,thiry2023}. ACOSUS addresses both issues through a dual-survey architecture that supports prediction while also consolidating advisor-facing information.
...
The architecture also supports flexible study designs through \textbf{survey linkage}, that is, explicit associations between Target and Factor Surveys. This makes it possible to compare different predictor sets, add new factors over time, and customize the system for different cohorts without changing the core infrastructure. Once these surveys are linked and deployed, the next step is to transform raw responses into model-ready inputs. Section~\ref{subsec:config} returns to the linkage mechanism from an implementation perspective.
...
These priority weights also remain useful in later modeling stages. When ACOSUS moves to probabilistic methods, the weights can be used as informative priors or as a principled starting point, so that features with higher priorities have greater initial influence before the model updates them using observed data. This preserves a link between expert knowledge and learned parameters. The next step in processing is to ensure that different kinds of survey responses are normalized appropriately.
...
\textbf{Foundation Stage.} The system focuses on collecting high-quality labeled observations through surveys that capture both outcomes and predictors. Once a minimum corpus exists, lightweight methods (e.g., k-nearest neighbors or cosine similarity matching) can generate initial predictions. At this stage, data quality matters more than model complexity.

\textbf{Augmentation Stage.} As the corpus grows, the system uses generative methods (e.g., SMOTE, generative adversarial networks, or variational autoencoders) to synthesize additional training observations. These methods learn from the real data and produce synthetic samples that expand the training set while preserving its main statistical properties.

\textbf{Refinement Stage.} Finally, the system transitions to more advanced non-linear models (e.g., deep neural networks, gradient boosting, or ensemble methods) that can capture complex relationships between features and outcomes.
...
\draw[black!60,dashed,thick] (target.south) -- node[right,font=\small,black]{Link} (fbox.north);
...
\midrule
Perceived Usefulness (PU) & 3.27 & 1.44 & 1 & 5 \\
Perceived Ease of Use (PEOU) & 3.83 & 1.47 & 1 & 5 \\
Likelihood to Continue & 3.00 & 2.07 & 0 & 5 \\
Likelihood to Recommend & 2.88 & 1.96 & 0 & 5 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{TAM construct scores ($N=10$). Perceived Ease of Use was the highest-rated TAM construct ($M=3.83$), while Perceived Usefulness was lower and showed similar variance ($M=3.27$).}
\label{fig:tam}
\end{figure}

Perceived Ease of Use received the highest mean among the TAM constructs ($M=3.83$, $SD=1.47$), indicating moderate, positive-leaning agreement that the system is easy to learn and use, though the wide spread points to uneven experiences across participants. Perceived Usefulness was lower and similarly dispersed ($M=3.27$, $SD=1.44$), suggesting that participants saw the system as easy to operate but were less uniformly convinced of its usefulness for their own advising needs.
...
\caption{Behavioral intention ($N=10$). Likelihood to Continue ($M=3.00$) was slightly higher than Likelihood to Recommend ($M=2.88$); both showed substantial variance across respondents ($SD\approx2$).}
\label{fig:behavioral}
\end{figure}

Behavioral intention was the weakest dimension in the evaluation. Likelihood to Continue using the system was moderate on average ($M=3.00$, $SD=2.07$), and Likelihood to Recommend was slightly lower and similarly dispersed ($M=2.88$, $SD=1.96$). Both distributions are bimodal: several respondents scored 5/5 while others scored 0--2, with few intermediate ratings. This pattern suggests that the system appeals strongly to a subset of transfer students but not to others, rather than producing uniform mild enthusiasm. We read this cautiously: with only ten respondents at the foundation phase, when no predictions had yet been generated, low recommendation scores may reflect limited current utility rather than rejection of the design.
...
\caption{Qualitative theme distribution ($N=10$). Feature Requests was the most frequent theme (8 mentions), followed by UI/Navigation Positive (7), No Issues Reported (5), and Assessment \& Exploration Value (4).}
\label{fig:themes}
\end{figure}
...
\midrule
Feature Requests & 8 & ``Make it tailored more towards individual users and their specific situation''; ``Better analysis of prerequisites to allow for registration'' \\
UI/Navigation Positive & 7 & ``The UI navigation is very user friendly''; ``All the interface of the website was very intuitive to interact with'' \\
...
Our primary contributions are: (1) a dual-survey architecture that separates outcome measurement from predictor collection while providing advisors with structured student profiles through a unified dashboard; (2) a modular prediction architecture with configuration support for survey linkage, phase transitions, and hyperparameters, allowing institutions to adapt the system to their own settings; and (3) a pilot evaluation with transfer students ($N=10$) showing that the system is perceived as moderately easy to use and useful, with Perceived Ease of Use ($M=3.83$) the strongest of the TAM constructs, alongside notable variance in behavioral intention.

%\textbf{Limitations and Current Status.} 
The system is currently operational and collecting observations during the foundation phase, but its predictive components still need to be evaluated as the dataset grows. % __ALT_PHRASING__: Conclusion limitations clause about participating
% institutions (active version below uses "small number of participating
% institutions" per recommended Option A). Alternative phrasing:
%   limited number of participating institutions (Option B wording).
The pilot's small sample size ($N=10$), small number of participating institutions, voluntary enrollment, and cross-sectional design limit generalizability, and longitudinal outcome tracking was beyond the scope of the present study.
